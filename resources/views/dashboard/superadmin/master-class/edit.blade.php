        ...
        <div class="page-heading">
            ...
            <section class="section">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{route('superadmin.master-class.update', ['id' => $masterClass->id])}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                            ...
                            <div class="row align-items-center mt-3">
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group mt-2">
                                        <input type="number" name="price" class="form-control" min="0" value="{{$masterClass->price ?? 0}}">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <div class="form-group mt-2">
                                        <select class="choices form-select" name="event_id">
                                            @foreach ($events as $event)
                                                <option value="{{ $event->id }}" {{$event->id == $masterClass->event_id ? 'selected' : ''}}>{{ $event->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-auto">
                                    <div class="form-check form-switch mt-4">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked" name="dashboard" {{$masterClass->active_dashboard ? 'checked' : ''}}>
